...
client = MongoClient('mongodb+srv://test:sparta@cluster0.yxgix5r.mongodb.net/?retryWrites=true&w=majority',
                     tlsCAFile=ca)
db = client.dbsparta
...
def todo_count():
    left_item = list(db.todo.find({'done': 0}, {'_id': False}))
    # left_item = list(db.todo.aggregate([
    #                                       {$match : {'done' : 0}},
    #
    #                                       ]))

    return jsonify({'lefts': left_item})
